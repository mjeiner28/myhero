...
def data_m():
    ...
    if form.validate_on_submit():
        pass_hass=bcrypt.generate_password_hash(form.password.data).decode('UTF-8')
        add_mahasiswa=Tmahasiswa(npm=form.npm.data, nama=form.nama.data, email=form.email.data, 
        password=pass_hass, kelas=form.kelas.data, alamat=form.alamat.data)
...
def edit_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        current_user.password=pass_hass
        db.session.commit()
...
def update_pengaduan(ed_id):
    ...
    if request.method=="GET":
        ...
        return redirect(url_for('rmahasiswa.pengaduan'))
    return render_template('editpengaduan.html', form=form)
